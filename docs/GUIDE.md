...
Content Factory - это 14-этапный автоматизированный конвейер для создания качественного контента.
...
9-12: Обогащение        → Факт-чек → Link placement → Translation → Editorial review
13-14: Публикация       → WordPress publication (опционально)
```
...
**9. Генерация по секциям**
- Последовательная генерация каждой секции
- 3 попытки на секцию с retry логикой

**10. Факт-чекинг (Fact-Check)**
- Google Gemini с нативным веб-поиском
- Проверка и исправление фактов
...
- Перевод на целевой язык (по умолчанию русский)
- Этап выполняется ВСЕГДА
- Сохранение HTML-структуры и ссылок

**13. Редакторский обзор (Editorial Review)**
...
- Автоматическая публикация (опционально)
