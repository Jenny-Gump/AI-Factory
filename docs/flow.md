...
- `merged_content.json` - объединенные секции с сырыми секциями
...
**ВХОДНЫЕ ДАННЫЕ:**
- `generated_sections[]` из этапа 9
- Промпт `prompts/basic_articles/09_fact_check.txt`
...
**Артефакты:**
- `09_fact_check/group_1/` до `group_N/` - проверенные группы секций
- `fact_checked_sections.json` - все проверенные секции
- `merged_fact_checked_content.json` - объединенная проверенная статья
...
**ВХОДНЫЕ ДАННЫЕ:**
- Проверенные секции из этапа 10 (факт-чек)
- `generated_sections[]` с fact_checked контентом
...
**Артефакты:**
- `content_with_links.json` - контент с ссылками для редактуры
- `link_placement_status.json` - статистика размещения
- `group_1/`, `group_2/` - обработанные группы секций
  - `llm_requests/link_placement_request.json`
  - `llm_responses_raw/link_placement_response.txt`
...
### Этап 12: Перевод контента (Translation) 🌍

**ВХОДНЫЕ ДАННЫЕ:**
- Контент с ссылками из этапа 11 (link placement)
- Переменная `--language` (целевой язык для перевода)
- `topic` пользователя для контекста
- Промпт `prompts/basic_articles/11_translation.txt` или `prompts/guides/11_translation.txt`
- FREE DeepSeek Chat v3.1 API credentials (via OpenRouter)

**🎯 ЦЕЛЬ:** Перевести статью с русского языка на целевой язык, сохранив всю HTML-разметку, ссылки, примеры кода и структуру

**УСЛОВИЕ ВЫПОЛНЕНИЯ:**
- ✅ **Этап выполняется ВСЕГДА** (обязательный этап пайплайна)
- `--language` указывает целевой язык (по умолчанию "русский")

**ФУНКЦИИ:**
- `src/llm_processing.py` → `translate_content()` - главная функция перевода
- FREE DeepSeek с температурой 0.3 для естественного перевода
- Автоматическое сохранение статуса перевода
- Graceful fallback на Gemini 2.5 при ошибках

**ПРОЦЕСС:**
1. **Получение целевого языка:**
   - Извлечение значения `--language` (default: "русский")
2. **Загрузка контента:** Чтение `merged_content_with_links.json` из папки link_placement
3. **Подготовка промпта:** Загрузка промпта с инструкциями по переводу
4. **LLM перевод** (выполняется всегда):
   - Перевод всего текстового контента на целевой язык
   - Сохранение ВСЕХ HTML-тегов без изменений
   - Перевод текста анкоров ссылок (сами URL сохраняются)
   - Сохранение примеров кода и команд терминала
   - Сохранение технических терминов (API, Framework, и т.д.)
5. **Валидация качества:** Проверка качества переведенного контента (длина, структура)
6. **Сохранение результата:**
   - `translated_content.json` - переведенный контент
   - `translation_status.json` - статус перевода (успех/ошибка, длина, целевой язык)

**ВЫХОДНЫЕ ДАННЫЕ:**
```json
{
  "title": "Translated Article Title",
  "content": "<h2>Translated Section</h2><p>Translated content...</p>",
...
**Артефакты:**
- `11_translation/translated_content.json` - переведенная статья
- `11_translation/translation_status.json` - статус перевода
- `11_translation/llm_requests/translation_request.json`
- `11_translation/llm_responses_raw/translation_response.txt`

**ОСОБЕННОСТИ ПЕРЕВОДА:**
- ✅ Переводится: Все текстовое содержимое (заголовки, абзацы, списки, FAQ, анкоры ссылок)
- ⛔ НЕ переводится: HTML-теги, URL, код, команды терминала, технические термины, названия продуктов
...
**СТАТИСТИКА:**
- **Время выполнения:** ~1-3 минуты (в зависимости от длины статьи)
- **Токены:** ~5-10k tokens (в зависимости от длины)
- **Модель:** FREE DeepSeek Chat v3.1 (primary) + Gemini 2.5 (fallback)
- **🔄 Graceful fallback**: При ошибках автоматическое переключение на Gemini 2.5

---

### Этап 13: Редакторская обработка (Editorial Review) ✏️

**ВХОДНЫЕ ДАННЫЕ:**
- Контент после перевода из этапа 12 (translation)
...
2. **🆕 Retry система с fallback:**
   - **Primary модель (DeepSeek):** 3 попытки с детальным логированием
   - **Fallback модель (Gemini 2.5):** 3 попытки при неудаче primary
...
3. **LLM редактура:** Выполняет:
   ...
   - Перевод англицизмов на русский
...
### Этап 14: WordPress Publication (WordPress Publication) 🌐

**ВХОДНЫЕ ДАННЫЕ:**
- Финальная отредактированная структура `wordpress_data_final.json` из этапа 13
...
Этап 8: all_structures → ultimate_structure.json (создание единой структуры)
           ↓
Этап 9: ultimate_structure + topic → generated_sections[] (черновые секции)
           ↓
Этап 10: generated_sections[] → fact_check_sections() → fact_checked_sections[] (проверенные секции)
           ↓
Этап 11: fact_checked_sections[] → place_links_in_sections() → content_with_links (контент со ссылками)
           ↓
Этап 12: content_with_links → translate_content() → translated_content (переведенный контент)
           ↓
Этап 13: translated_content → editorial_review() → wordpress_data_final.json (отформатированная статья)
           ↓
Этап 14: wordpress_data_final.json → WordPress API → draft post (опционально)
```

### **Обработка ошибок (ОБНОВЛЕНО - Sep 27, 2025):**
- **🆕 Продвинутая retry система:** 3×3 попытки (primary + fallback модели)
- **🆕 4-уровневая JSON нормализация** с детальным логированием каждой попытки
...
1. **Простота и эффективность**: Полный 14-этапный пайплайн с разделением обязанностей
...
4. **🆕 РАЗДЕЛЬНАЯ ОБРАБОТКА**:
   - **Этап 10**: Google Gemini проверяет ФАКТЫ через нативный веб-поиск
   - **Этап 11**: Google Gemini находит и добавляет ССЫЛКИ через нативный веб-поиск
   - **Этап 12**: DeepSeek переводит контент на целевой язык
   - **Этап 13**: DeepSeek форматирует и полирует СТРУКТУРУ
5. **Прямая публикация**: Этап 14 сразу публикует готовый контент в WordPress
6. **Полная прозрачность**: Комплексное логирование всех LLM и WordPress взаимодействий
7. **Многоязычность**: Автоматический перевод на любой язык (по умолчанию русский)
...
Эта полная 14-этапная архитектура обеспечивает разделение ответственности: **Gemini проверяет факты + находит ссылки (веб-поиск)**, **DeepSeek генерирует + переводит + форматирует (FREE)**, что гарантирует высочайшее качество контента при оптимальной стоимости.
