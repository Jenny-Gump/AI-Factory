...
**Артефакты:**
- `wordpress_data_final.json` - финальная отредактированная статья с исправленными переносами строк
...
Этап 10: fact_checked_sections[] → editorial_review() → wordpress_data_final.json (отформатированная статья)
           ↓
Этап 11: wordpress_data_final.json → WordPress API → draft post (опционально)
```
...
1. **Простота и эффективность**: Полный 11-этапный пайплайн с разделением факт-чека и редактуры
...
5. **Прямая публикация**: Этап 11 сразу публикует готовый контент в WordPress
...
Эта полная 11-этапная архитектура обеспечивает разделение ответственности: **Perplexity проверяет факты**, **DeepSeek форматирует**, что гарантирует высочайшее качество контента при оптимальной стоимости.
