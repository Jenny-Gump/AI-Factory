...
### Этап 10: Расстановка ссылок (Link Placement) 🔗

**ВХОДНЫЕ ДАННЫЕ:**
- Проверенные секции из этапа 9.5 (факт-чек)
...
- Firecrawl Search API для поиска качественных источников
- FREE DeepSeek с fallback на Gemini 2.5 при таймаутах
- 3-секундные паузы между группами для rate limits
...
### Этап 11: Редакторская обработка (Editorial Review) ✏️

**ВХОДНЫЕ ДАННЫЕ:**
- Контент с ссылками из этапа 10 (link placement)
...
### Этап 12: WordPress Publication (WordPress Publication) 🌐

**ВХОДНЫЕ ДАННЫЕ:**
- Финальная отредактированная структура `wordpress_data_final.json` из этапа 11
...
Этап 9.5: generated_sections[] → fact_check_sections() → fact_checked_sections[] (проверенные секции)
           ↓
Этап 10: fact_checked_sections[] → place_links_in_sections() → content_with_links (контент со ссылками)
           ↓
Этап 11: content_with_links → editorial_review() → wordpress_data_final.json (отформатированная статья)
           ↓
Этап 12: wordpress_data_final.json → WordPress API → draft post (опционально)
```
...
1. **Простота и эффективность**: Полный 12-этапный пайплайн с разделением обязанностей
...
4. **🆕 РАЗДЕЛЬНАЯ ОБРАБОТКА**:
   - **Этап 9.5**: Google Gemini проверяет ФАКТЫ через нативный веб-поиск
   - **Этап 10**: DeepSeek добавляет ССЫЛКИ на авторитетные источники
   - **Этап 11**: DeepSeek форматирует и полирует СТРУКТУРУ
5. **Прямая публикация**: Этап 12 сразу публикует готовый контент в WordPress
...
8. **🆕 ОПТИМИЗИРОВАННАЯ СТОИМОСТЬ**:
   - Google Gemini ТОЛЬКО для факт-чека (где нужен нативный веб-поиск)
   - FREE DeepSeek для всех остальных этапов (структуры, генерация, ссылки, редактура)
...
13. **🆕 КАЧЕСТВО КОНТЕНТА**: Каждая секция проходит факт-проверку + обогащается ссылками

Эта полная 12-этапная архитектура обеспечивает разделение ответственности: **Gemini проверяет факты**, **DeepSeek генерирует + добавляет ссылки + форматирует**, что гарантирует высочайшее качество контента при оптимальной стоимости.
