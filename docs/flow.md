...
This document provides a detailed, step-by-step breakdown of the complete 11-stage content generation pipeline for WordPress article generation.
...
### Этап 9.5: Факт-чекинг секций (Fact-checking) 🔍

**ВХОДНЫЕ ДАННЫЕ:**
- `generated_sections[]` из этапа 9
- Промпт `prompts/basic_articles/09_fact_check.txt`
- `topic` для контекстуализации
- Perplexity Sonar Reasoning Pro API (с веб-поиском)

**🎯 ЦЕЛЬ:** Проверить фактическую достоверность каждой секции через веб-поиск

**ФУНКЦИИ:**
- `src/llm_processing.py` → `fact_check_sections()` - главная функция
- Perplexity с обязательным веб-поиском для проверки фактов
- 3-секундные паузы между запросами
- Индивидуальная обработка каждой секции

**ПРОЦЕСС:**
1. **Фильтрация:** Отбор успешно сгенерированных секций
2. **Поочередная проверка:** Для каждой секции:
   - Отправка контента в Perplexity с промптом факт-чекера
   - Обязательное выполнение 5-10 веб-поисков
   - Проверка версий, дат, статистики, ссылок
   - Исправление галлюцинаций и устаревших данных
...
[
  {
    ...
    "fact_check_model": "perplexity/sonar-reasoning-pro:online"
  }
]
```

**Артефакты:**
- `09_fact_check/section_1/` до `section_N/` - проверенные секции
- `fact_checked_sections.json` - все проверенные секции
- `merged_fact_checked_content.json` - объединенная проверенная статья

**КЛЮЧЕВЫЕ ОСОБЕННОСТИ:**
- **Обязательный веб-поиск:** Perplexity автоматически использует поиск
- **Сохранение оригинала:** Для сравнения и отката
- **Graceful fallback:** При ошибке сохраняется оригинальная секция
- **Детальное логирование:** Каждая секция в отдельной папке
...
**ВХОДНЫЕ ДАННЫЕ:**
- Проверенная структура из этапа 9.5 (факт-чек)
...
5. **Специальная обработка моделей:** Gemini-specific cleanup для корректного парсинга
...
### **Запуск пайплайна:**
```bash
# Полный пайплайн с генерацией и редактурой (этапы 1-10)
python main.py "Лучшие промпты для создания идей для видео"

# Полный пайплайн с публикацией (этапы 1-11)
python main.py "Промпты для анализа данных" --content-type basic_articles
...
Этап 9: ultimate_structure + topic → generated_sections[] (черновые секции)
           ↓
Этап 9.5: generated_sections[] → fact_check_sections() → fact_checked_sections[] (проверенные секции)
           ↓
Этап 10: fact_checked_sections[] → editorial_review() → wordpress_data_final.json (отформатированная статья)
           ↓
Этап 11: wordpress_data_final.json → link_processing() → wordpress_data_with_links.json (+ ссылки)
           ↓
Этап 12: wordpress_data_with_links.json → WordPress API → draft post (опционально)
```
...
### **Ключевые принципы дизайна (ОБНОВЛЕНО - Этап 9.5):**
1. **Простота и эффективность**: Полный 12-этапный пайплайн с разделением факт-чека и редактуры
...
4. **🆕 РАЗДЕЛЬНАЯ ОБРАБОТКА**:
   - **Этап 9.5**: Perplexity проверяет ФАКТЫ через веб-поиск
   - **Этап 10**: DeepSeek форматирует и полирует СТРУКТУРУ
5. **Академическое обогащение**: Этап 11 добавляет авторитетные ссылки и источники
6. **Полная прозрачность**: Комплексное логирование всех LLM и WordPress взаимодействий
7. **Русскоязычный фокус**: Специализация на российскую аудиторию и сайт ailynx.ru
8. **🆕 ОПТИМИЗИРОВАННАЯ СТОИМОСТЬ**:
   - Perplexity ТОЛЬКО для факт-чека (где нужен веб-поиск)
   - FREE DeepSeek для всех остальных этапов
9. **SEO-оптимизация**: Автоматическое заполнение Yoast SEO полей для поисковых систем
10. **Безопасная публикация**: Статьи создаются в статусе draft для проверки перед публикацией
11. **Практическая ценность**: Фокус на реальных случаях использования и экспертных рекомендациях
12. **🆕 КАЧЕСТВО КОНТЕНТА**: Каждая секция проходит индивидуальную факт-проверку перед объединением

Эта полная 12-этапная архитектура обеспечивает разделение ответственности: **Perplexity проверяет факты**, **DeepSeek форматирует**, что гарантирует высочайшее качество контента при оптимальной стоимости.
