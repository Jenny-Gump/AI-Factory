...
- ✅ **Валидация v3.0:** 6-уровневая научная проверка (compression, entropy, bigrams, finish_reason, language check)
...
Этап 12: content_with_links → editorial_review() → wordpress_data_final.json (отформатированная статья)
           ↓
Этап 13: wordpress_data_final.json → WordPress API → draft post (опционально)
```

### **Обработка ошибок (ОБНОВЛЕНО - Oct 6, 2025):**
- **🆕 Единая система валидации контента (v2.3.0):**
  - Валидация происходит ВНУТРИ `_make_llm_request_with_retry_sync()`
  - Провал валидации (спам, gibberish, < 300 chars) → exception → retry → fallback
  - Retry Flow: Primary model (3 попытки) → Fallback model (3 попытки) = 6 attempts
  - Применяется на ВСЕХ LLM этапах (7-12)
